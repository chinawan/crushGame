...
function RuleLayer:ctor()
  ...
  --泡泡的列表容器  和 选择列表
    ...
    self.score = 0
    local label = cc.ui.UILabel.new({text = "Score: ",size = 64}):pos(100,750):addTo(self)
    self.scoreLabel = cc.ui.UILabel.new({text = "0",size = 64}):pos(300,750):addTo(self):setAnchorPoint(0,0.5)
...
function RuleLayer:swap( bubbleA, bubbleB )
    ...
    if self:checkAndExpode(true) == false  then
	    ...
	    local sequA = cc.Sequence:create(
	      					cc.CallFunc:create(function() 
	                            bubbleA:moveToGrid(bPosX, bPosY)
	                        end),
	                        cc.DelayTime:create(0.3), 
	                        cc.CallFunc:create(function() 
	                            bubbleA:moveToGrid(aPosX, aPosY,true)
	                        end))
	    bubbleA:runAction(sequA)

	    local sequB = cc.Sequence:create(
	      					cc.CallFunc:create(function() 
	                            bubbleB:moveToGrid(aPosX, aPosY)
	                        end),cc.DelayTime:create(0.3), 
	                        cc.CallFunc:create(function() 
	                        	bubbleB:moveToGrid(bPosX, bPosY,true)
	                        end))
...
function RuleLayer:checkAndExpode(isCheck)
    ...
    local counter=1  --相同数量
    local bMark=0    --交换泡泡的ID
    local starIndex=1 --起始
    --[[检测行]]
    for row=1 ,self.RowCount do
    	bMark =  getGridID(1,row)
    	starIndex=1
	    counter=1
...
function  RuleLayer:explodeBubble()
    ...
    for i=1,#self.explodeList do
	    ...
	    explodePati:pos(posX, posY):setAutoRemoveOnFinish(true):addTo(self,6)

	    local x, y =self.explodeList[i].posX , self.explodeList[i].posY
	    self.gridMap[x][y] = nil
	    self.explodeList[i]:pos(1,3000)
	    self.explodeList[i]:stopAllActions()
	    self.explodeList[i]:setScale(1)
    end
